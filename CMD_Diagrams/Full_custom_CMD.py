...
def full_area():
    """
    Функция построения полной CMD из выбранных областей галактики
    """
    # Отбираем хорошие звезды из выбранных областей галактики с помощью эллипса и дуг юбочек
    with open('good_stars.txt', 'r', encoding='UTF-8') as good:
        with open('good_stars_final.txt', 'w', encoding='UTF-8') as final_file:
            ...
            while line := good.readline():
                ...
                if float(numbers[9]) < 0.5 and float(numbers[10]) <= 2.0 and float(numbers[21]) >= 5.0 and float(
                    ...
                    numbers[40]) <= 2.0 and 1480.0 <= float(numbers[2]) <= 2027.0 or 1840.0 <= float(
                    numbers[2]) <= 2578.0 or 2409.0 <= float(numbers[2]) <= 3014.0 or 3519.0 <= float(
                    numbers[3]) <= 4153.0 or 2458.0 <= float(numbers[3]) <= 3519.00 or 1918.0 <= float(
                    numbers[3]) <= 2596.0:
