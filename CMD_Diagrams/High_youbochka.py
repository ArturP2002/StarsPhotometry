...
def high_area():
    """
    Функция для записи в файл всех хороших звезд из верхней юбочки, построения CMD верхней юбочки
    """
    # Отбираем хорошие звезды верхней юбочки
    with open('good_stars.txt', 'r', encoding='UTF-8') as good:
        with open('good_stars_high.txt', 'w', encoding='UTF-8') as final_file:
            ...
            while line := good.readline():
                ...
                if float(numbers[9]) < 0.5 and float(numbers[10]) <= 2.0 and float(numbers[21]) >= 5.0 and float(
                        ...
                        numbers[40]) <= 2.0 and 2409.0 <= float(numbers[2]) <= 3014.0 and 1918.0 <= float(
                        numbers[3]) <= 2596.0:
