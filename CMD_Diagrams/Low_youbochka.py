...
def low_area():
    """
    Функция для записи в файл всех хороших звезд из нижней юбочки, построения CMD нижней юбочки
    """
    # Отбираем хорошие звезды нижней юбочки
    with open('good_stars.txt', 'r', encoding='UTF-8') as good:
        with open('good_stars_low.txt', 'w', encoding='UTF-8') as final_file:
            ...
            while line := good.readline():
                ...
                if float(numbers[9]) < 0.5 and float(numbers[10]) <= 2.0 and float(numbers[21]) >= 5.0 and float(
                    ...
                    numbers[40]) <= 2.0 and 1480.0 <= float(numbers[2]) <= 2027.0 and 3519.0 <= float(
                    numbers[3]) <= 4153.0:
